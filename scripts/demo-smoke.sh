...
external_alert_id="$(new_external_alert_id smoke)"
...
decide_approval \
  ...
  "$tool_call_id" \
  APPROVE \
  "Task 14 deterministic happy-path approval"
wait_task_completed "$task_id"

evidence="$(
  http_get "$API_BASE_URL/api/acceptance/tasks/$task_id"
)"
summary="$(printf '%s' "$evidence" | assert_acceptance smoke "$task_id")"

printf 'scenario=smoke externalAlertId=%s taskId=%s\n' \
